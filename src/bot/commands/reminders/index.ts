...
import { postToResponseUrl } from "../shared/index.js";
import {
  getReminderSettings,
...
const reminders: CommandModule = {
  name: "reminders",
  register(app) {
    app.command("/reminders", async ({ command, ack, respond }) => {
      await ack();
    ...
    app.action<BlockAction>(REMINDERS_TOGGLE_ACTION_ID, async ({ ack, body, action }) => {
      await ack();

      const teamId = resolveTeamId(body);
      const nextValue = "value" in action ? action.value === "on" : null;
    ...
    app.action<BlockAction>(REMINDERS_CONFIGURE_ACTION_ID, async ({ ack, body, client }) => {
      await ack();
    ...
    app.view(REMINDERS_CONFIG_MODAL_CALLBACK_ID, async ({ ack, body, view }) => {
